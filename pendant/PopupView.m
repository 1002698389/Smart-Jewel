...
- (void) setText1:(NSString *) text
{
    ...
    CGRect frame = CGRectMake(5, 10, _textLabel.frame.size.width, _textLabel.frame.size.height);
...
- (void) setText1:(NSString *)text :(CGRect)myFrame
{
    _textLabel.frame = myFrame;
    _queueCount ++;
    self.alpha = 0.0f;
    _textLabel.text = text;
    [_textLabel sizeToFit];
    CGRect frame = CGRectMake(5, 10, _textLabel.frame.size.width-10, _textLabel.frame.size.height);
    _textLabel.frame = frame;
    frame =  CGRectMake((_parentView.frame.size.width - frame.size.width)/2, self.frame.origin.y, _textLabel.frame.size.width, _textLabel.frame.size.height+10);
    self.frame = frame;
    [UIView animateWithDuration:3.0 delay:0.0 options:UIViewAnimationOptionCurveEaseIn animations:^{
        self.alpha = 1; } completion:^(BOOL finished){
             //                         if (_queueCount == 10) {
             //                             [self removeFromSuperview];
             //                         }
             //                         _queueCount--;
